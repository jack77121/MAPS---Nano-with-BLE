...
public class HM10Activity extends Activity {
    ...
    private TextView TemperatureReceived;
    private TextView HumidityReceived;
    private TextView CO2Received;
    private TextView PM25Received;
...
//        mButtonReset= (Button) findViewById(R.id.buttonReset);
        ...
        TemperatureReceived = (TextView) findViewById(R.id.Temperature);
        HumidityReceived = (TextView) findViewById(R.id.Humidity);
        CO2Received = (TextView) findViewById(R.id.CO2_value);
        PM25Received = (TextView) findViewById(R.id.PM25_value);

        time = (TextView) findViewById(R.id.myupdatetime);
...
//			}
            String[] myStr = text.split(",");
//			mTextLongReceived.setText(text);
			if(myStr[0].contentEquals("tmp")){
				TemperatureReceived.setText(myStr[1]);
			}
			else if(myStr[0].contentEquals("hmd")){
				HumidityReceived.setText(myStr[1]);
			}
			else if(myStr[0].contentEquals("pm25")){
				PM25Received.setText(myStr[1]);
			}
			else if(myStr[0].contentEquals("co2")){
				CO2Received.setText(myStr[1]);
			}
			else{
				// nothing
			}
    ...
    private class HM10BroadcastReceiver extends BroadcastReceiver {
        ...
        private String sendPostData(String dataStr){
            HttpPost httpRequest = new HttpPost("http://nrl.iis.sinica.edu.tw/jack/android_demo.php");
            ...
            try{
                ...
                Toast.makeText(mContext, "error 01", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
            catch(IOException e){
                Toast.makeText(mContext, "error 02", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
            catch(Exception e){
                Toast.makeText(mContext, "error 03", Toast.LENGTH_SHORT).show();
                e.printStackTrace();
            }
       ...
       public void onReceive(Context context, Intent intent) {
           ...
           if (intent.getAction().equals(TumakuBLE.READ_SUCCESS)) {
        	   ...
        	   if (mState==WSTATE_READ_KEY) {
        		   if (readByteArrayValue!=null) displayText("start");
        		   mState=WSTATE_DUMMY;
        		   nextState();
        		   return;
        	   }
...
//                       sendPostData(tmpString);
                       updateLongText(tmpString);
                       mCalendar = Calendar.getInstance();
                       df = new SimpleDateFormat("yyyy/MM/dd  HH:mm:ss");

                       myTimestr = df.format(mCalendar.getTime());
